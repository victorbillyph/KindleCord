...
cleanup() {
    ...
    iptables -D INPUT -p tcp --dport 2222 -j ACCEPT 2>> "$LOG"
...
echo "Opening firewall port 8080,2222..." >> "$LOG"
iptables -C INPUT -p tcp --dport 8080 -j ACCEPT 2>> "$LOG" || \
    iptables -A INPUT -p tcp --dport 8080 -j ACCEPT 2>> "$LOG"
iptables -C INPUT -p tcp --dport 2222 -j ACCEPT 2>> "$LOG" || \
    iptables -A INPUT -p tcp --dport 2222 -j ACCEPT 2>> "$LOG"
